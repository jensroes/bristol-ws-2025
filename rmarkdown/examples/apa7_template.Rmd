...
This is a reanalysis of data published in @blomkvist2017reference.
...
\begin{equation}
    (\#eq:lm)
    \begin{aligned}
      \mu_i = \beta_0 + \beta_1 \cdot age_i + \beta_2 \cdot age_i^2 + \beta_3 \cdot smoker_i + \\
      \beta_4 \cdot age_i \cdot smoker_i + \beta_5 \cdot age_i^2 \cdot smoker_i
    \end{aligned}
\end{equation}
```


\noindent with data $y$ coming from a normal distribution $y_i \sim \mathcal{N}(\mu_i, \sigma^2)$ with a mean $\mu$ and a standard deviation $\sigma^2$ for $i \in 1 \dots N$ where $N$ is the total number of observations.
...
lrt_fin_model <- tidy_lrt %>% 
  ...
  transmute(term = paste0("*F*(", df, ", ", df.residual, ") = ", statistic, ", *p*", format.pval(p.value, eps = 0.05)))
```
...
tidy_lrt_pretty <- tidy_lrt %>% 
  mutate(Model = c("M$_0$", "M$_{age}$", "M$_{age^2}$", "M$_{smoker}$", "M$_{interaction}$"),
         ...
         across(p.value, ~format.pval(., eps = 0.05)),
         across(everything(), ~as.character(.)),
         across(everything(), ~str_replace(., "NA", "")),
...
Model coefficients of the best fitting model can be found in Table \ref{tab:modelcoefs}.
...
Looks like smoking makes you slower in advanced ages.
