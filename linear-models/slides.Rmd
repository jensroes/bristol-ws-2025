...
You will need to complete the code in those scripts. Observe how changing the number of observations affects the model estimates.
...
```{r eval = F, echo = F}
saveRDS(object = fit_brm, file = "../stanout/fit_brm.rds", compress = "xz")
```


```{r eval = T, echo = F}
fit_brm <- readRDS(file = "../stanout/fit_brm.rds")
```
...
```{r echo = T}
beta <- pas_draws_df(fit_brm) %>% pull(b_nptypesimple)
```
...
- See exercises: `linear-models/exercises/hypothesis_testing_1.R`
...
- Regularising (skeptical) priors prevent the model to get overexcited by the sample [his words: @mcelreath2016statistical].
...
- Explore `hypothesis()` in the exercise: `linear-models/exercises/hypothesis_testing_1.R`
- Bonus: explore the Savage-Dickey method in `linear-models/exercises/hypothesis_testing_3.R`
...
- other probability models: lognormal, shifted_lognormal, mixture, probably as demo
- show changes in fit
- model comparisons and cross-validation
