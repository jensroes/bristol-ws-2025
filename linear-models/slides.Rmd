...
- Models with a combination of fixed (intercepts and slopes) and random effects: repeated measures ANOVA are a special type of mixed-effects models.
...
- Allows to address multi-level (hierarchical) and crossed structure in the data.
...
- Linear mixed-effects models can capture both (and more) [@baayen2008mixed].
...
## Crossed random intercepts and random slopes model
...
\
 ...
 Two different ways of generalising from sample to the population: What do the data tell us about the truth?
...
## Its problems {.columns-2 .smaller}

- People think of statistical significance as continuous not binary.
- How often do we actuallybelieve in $H_0$?
...
- Complete script `mixed_effects_model_brms_with_priors.R` 
...
- Define region that is equivalent to no effect: values in interval of [-5, 5] msecs
